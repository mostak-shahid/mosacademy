		...
		<?php if($page_layout != 'ns') : ?>
			...
			<?php endif; ?>
				...
				<?php if ( have_posts() ) :?>
					...
					$gallery_gap = get_post_meta( get_the_ID(), '_mosacademy_multy_gallery_gap', true );
					$per_page = (get_post_meta( get_the_ID(), '_mosacademy_multy_gallery_image_per_page', true )) ? get_post_meta( get_the_ID(), '_mosacademy_multy_gallery_image_per_page', true ) : '-1' ;

					$large_image_size = (get_post_meta( get_the_ID(), '_mosacademy_multy_gallery_large_image_size', true )) ? get_post_meta( get_the_ID(), '_mosacademy_multy_gallery_large_image_size', true ) : 'container' ;
					$image_width = (get_post_meta( get_the_ID(), '_mosacademy_multy_gallery_image_width', true )) ? get_post_meta( get_the_ID(), '_mosacademy_multy_gallery_image_width', true ) : '263' ;
					$image_height = (get_post_meta( get_the_ID(), '_mosacademy_multy_gallery_image_height', true )) ? get_post_meta( get_the_ID(), '_mosacademy_multy_gallery_image_height', true ) : '263' ;


					$gallery_details_group = get_post_meta( get_the_ID(), '_mosacademy_multy_gallery_details_group', true );
					...
					<?php if ($gallery_details_group) : ?>
						...
						<?php foreach ($gallery_details_group as $gallery_details_value) : ?>
							<?php
							$top_title = $gallery_details_value["_mosacademy_multy_gallery_title_text"];
							$top_title_img = wp_get_attachment_url($gallery_details_value["_mosacademy_multy_gallery_title_images_id"]);

							$tab_details = $gallery_details_value["_mosacademy_multy_gallery_tab_details"];
							//var_dump($tab_details);
							// $tab_names = rtrim($gallery_details_value["_mosacademy_multy_gallery_tab_names"], ',');
							// $tab_name_arr = explode(",",$tab_names);
							if (@$gallery_details_value["_mosacademy_multy_gallery_tab_images"][0]) : 
							?>
							...
							<?php endif; ?>
								...
								<?php foreach ($tab_details as $tab_detail) : ?>
									...
									<?php if ($gallery_details_value["_mosacademy_multy_gallery_tab_images"]) : ?>
										<div id="group_gallery_<?php echo $active ?>" class="row">
										<?php foreach ($gallery_details_value["_mosacademy_multy_gallery_tab_images"][$active] as $attachment_id => $url) : ?>
