<?php 
function mosacademy_custom_metabox () {
	//add_meta_box( $id, $title, $callback, $screen = null, $context = 'advanced', $priority = 'default', $callback_args = null );

	add_meta_box( 
		'_mosacademy_custom_metabox', 
		'mosacademy Custom Metabox', 
		'mosacademy_custom_metabox_html', 
		array('post','page'),
...
add_action( 'add_meta_boxes', 'mosacademy_custom_metabox' );

function mosacademy_custom_metabox_html ($post) { 
	// Add a nonce field so we can check for it later.
	wp_nonce_field( 'mosacademy_save_custom_metabox', 'mosacademy_custom_metabox_nonce' );
...
function mosacademy_custom_metabox_update ($post_ID) {

	if ( ! isset( $_POST['mosacademy_custom_metabox_nonce'] ) ) {
		return;
	}

	if ( ! wp_verify_nonce( $_POST['mosacademy_custom_metabox_nonce'], 'mosacademy_save_custom_metabox' ) ) {
		return;
	}
...
add_action( 'save_post', 'mosacademy_custom_metabox_update' );