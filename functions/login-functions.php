<?php
/*Small Changes*/
function mosacademy_login_logo() { 
	global $mosacademy_options;
	?>
    <style type="text/css">
        #login h1 a, .login h1 a {
            background-image: url(<?php echo wp_get_attachment_url( $mosacademy_options["logo"]["id"] )?>);
            width: 100%;
            max-width: 320px;
            height:<?php echo $mosacademy_options["logo"]["height"] ?>px;
    		background-size: contain;
    		background-repeat: no-repeat;
        }
    </style>
<?php }
add_action( 'login_enqueue_scripts', 'mosacademy_login_logo' );

function mosacademy_login_logo_url() {
    return home_url();
}
add_filter( 'login_headerurl', 'mosacademy_login_logo_url' );

function mosacademy_login_logo_url_title() {
    return get_bloginfo( 'name' ) . ' - ' . get_bloginfo( 'description' );
}
add_filter( 'login_headertitle', 'mosacademy_login_logo_url_title' );


/*Adding Css and JS*/
function mosacademy_login_stylesheet() {
...
add_action( 'login_enqueue_scripts', 'mosacademy_login_stylesheet' );
...
add_action( 'register_form', 'mosacademy_register_form' );
function mosacademy_register_form() {
...
add_filter( 'registration_errors', 'mosacademy_registration_errors', 10, 3 );
function mosacademy_registration_errors( $errors, $sanitized_user_login, $user_email ) {
...
add_action( 'user_register', 'mosacademy_user_register' );
function mosacademy_user_register( $user_id ) {
...
/*Login redirect*/
function mosacademy_login_redirect( $redirect_to, $request, $user  ) {
...
add_filter( 'login_redirect', 'mosacademy_login_redirect', 10, 3 );

function mosacademy_single_post_redirect () {
...
add_action( 'wp_head', 'mosacademy_single_post_redirect' );

/*Limit admin access*/
function mosacademy_blockusers_init() {
    if ( is_admin() && !current_user_can( 'administrator' ) && !( defined( 'DOING_AJAX' ) && DOING_AJAX ) ) {
        wp_redirect( home_url('/dashboard/') );
        exit;
    }
}
add_action( 'init', 'mosacademy_blockusers_init' );