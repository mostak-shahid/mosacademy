...
$feature = $mosacademy_options['sections-testimonial-feature']; 
$feature_width = $mosacademy_options['sections-testimonial-feature-width']; 
$feature_details = $mosacademy_options['sections-testimonial-feature-details']; 

$layout = $mosacademy_options['sections-testimonial-layout'];
$view = $mosacademy_options['sections-testimonial-view'];
...
<section id="section-testimonial" <?php if ($animation) echo 'data-wow-delay="'.$animation_delay.'s" class="wow '.$animation.'"' ?>>
	<div class="content-wrap">
		...
		*/
		...
		<?php
		$count = ($mosacademy_options['sections-testimonial-count']) ? $mosacademy_options['sections-testimonial-count'] : '-1' ;
		$args = array(
		...
		<?php if ($query -> have_posts()) : ?>	
			...
			while ($query -> have_posts()) : $query -> the_post(); 
				$designation = get_post_meta( get_the_ID(), '_mosacademy_testimonial_designation', true );
				$image = get_post_meta( get_the_ID(), '_mosacademy_testimonial_image', true );
				$oembed = get_post_meta( get_the_ID(), '_mosacademy_testimonial_oembed', true );
				?>
					<div class="<?php if ($view == 'grid') echo 'col-md-'.$colsize; else echo 'wrapper'?>">
		                <div class="testimonial-content<?php if ($mosacademy_options['sections-testimonial-content-layout'] == 2) echo ' row' ?>">
		                	<?php
		                	if ($mosacademy_options['sections-testimonial-content-layout'] == 2 AND $mosacademy_options['sections-testimonial-section1-width']) :
			                	$sec_1 = $mosacademy_options['sections-testimonial-section1-width'];
			                	$slice = explode("-",$sec_1);
			                	$num = 12 - end($slice);
		                	...
		                	<?php foreach ($mosacademy_options['sections-testimonial-section1'] as $shortcodes) : ?>
		                		<?php echo do_shortcode( $shortcodes ) ?>
		                	<?php endforeach; ?>
		                	</div>
		                	<div class="sec-2 <?php  echo $sec_2 ?>">		                		
		                	<?php foreach ($mosacademy_options['sections-testimonial-section2'] as $shortcodes) : ?>
		                		<?php echo do_shortcode( $shortcodes ) ?>
		                	<?php endforeach; ?>
		                	</div>  
		                </div> 
