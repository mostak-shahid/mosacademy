...
$animation = $mosacademy_options['sections-blog-animation'];
$animation_delay = ( $mosacademy_options['sections-blog-animation-delay'] ) ? $mosacademy_options['sections-blog-animation-delay'] : 0;
$view = $mosacademy_options['sections-blog-view'];
...
?>
<?php
$count = ($mosacademy_options['sections-blog-count']) ? $mosacademy_options['sections-blog-count'] : '-1' ;
$args = array(
		...
		<?php if ($query -> have_posts()) : ?>
			...
			<?php while ( $query -> have_posts() ) : $query -> the_post(); ?>
				<div class="<?php if ($view == 'grid') echo 'col-sm-'.$smallcol.' col-md-'.$colsize; else echo 'wrapper'?>">
		                <div class="blog-content<?php if ($mosacademy_options['sections-blog-content-layout'] == 2) echo ' row' ?>">
		                	<?php
		                	if ($mosacademy_options['sections-blog-content-layout'] == 2 AND $mosacademy_options['sections-blog-section1-width']) :
			                	$sec_1 = $mosacademy_options['sections-blog-section1-width'];
			                	$slice = explode("-",$sec_1);
			                	$num = 12 - end($slice);
		                	...
		                	<?php if ($mosacademy_options['sections-blog-section1']) : ?>
		                	<?php foreach ($mosacademy_options['sections-blog-section1'] as $shortcodes) : ?>
		                		<?php echo do_shortcode( $shortcodes ) ?>
		                	<?php endforeach; ?>
		                	<?php endif; ?>
		                	</div>
		                	<div class="sec-2 <?php  echo $sec_2 ?>">	
		                	<?php if ($mosacademy_options['sections-blog-section2']) : ?>	                		
		                	<?php foreach ($mosacademy_options['sections-blog-section2'] as $shortcodes) : ?>
		                		<?php echo do_shortcode( $shortcodes ) ?>
		                	<?php endforeach; ?>
		                	<?php endif; ?>
		                	</div>  
